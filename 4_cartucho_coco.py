...
parser.add_argument('--outdir', help='Output folder to save results in', default='chartucho')
...
parser.add_argument('--show_images', help='Display and save images as they are evaluated',
                    action='store_true')  # Coming soon!
parser.add_argument('--show_plots', help='Display and save plots showing precision/recall curve, mAP score, etc',
                    action='store_true')  # Coming soon!
...
classes = ['passenger_ship', 'cargo_ship', 'special_ship', 'boat', 'buoy']

# Make folder to store output result files
if os.path.exists(output_path):
    print(
        'The output folder %s already exists. Please delete it or specify a different folder name using --outdir.' % output_path)
    sys.exit()
else:
...
for i, iou_thresh in enumerate(iou_threshes):
    ...
    with open('3_cartucho_map.py', 'r') as f:
        data = f.read()
